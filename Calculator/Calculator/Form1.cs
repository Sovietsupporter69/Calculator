...
namespace Calculator
{
    public partial class Form1 : Form
    {
        ...
        void Calculate()
        {
            Input = OutputBox.Text; //Changing the input into an array of strings in the calculation order
            SplitInput = Input.ToCharArray(); //split the string into individual characters
            BracketSplit();
        }
        ...
        void BracketSplit()
        {
            for (int i = 0; i < SplitInput.Length; i++)
            {
                if (SplitInput[i] == '(') //searches the character array for a left bracket
                {
                    ...
                    Hold[i] = new int[2] { Priority, 0 }; //and asigns that to the bracket in the hold array
                    BracketSplit();
                }
